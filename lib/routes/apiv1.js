...
function getKeyDetailsHash (key, redisConnection, res, next) {
  redisConnection.hgetall(new Buffer(key), function (err, fieldsAndValues) {
    ...
    var details = {
      key: key,
      type: 'hash',
      data:ObjectBinaryToString(fieldsAndValues)
    };
    myutil.encodeHTMLEntities(JSON.stringify(details), function (string) {
      res.send(string);
    })
...
// Gets an object containing binary variables and parse it to an object containing string
function ObjectBinaryToString(object){
  Object.keys(object).forEach(function(place){
    var binary = object[place].toJSON()['data'];
    var srtingJson = BERT.bytelist_to_string(binary);
    var newObj = BERT.decode(srtingJson);
    object[place] = BERT.repr(newObj)
  })
  return object
}
