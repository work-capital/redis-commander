...
function getKeyDetailsHash (key, redisConnection, res, next) {
  redisConnection.hgetall(new Buffer(key), function (err, fieldsAndValues) {
    ...
    try {
      var newData = convertBertToString(fieldsAndValues);
    } catch (e) {
      var newData = fieldsAndValues.toString();
    };
