...
var bert_erlastic = require('./node_erlastic');
  ...
  getConnection(req.redisConnections, connectionId, function (err, redisConnection) {
    ...
    redisConnection.type(key, function (err, type) {
      ...
      switch (type) {
        ...
        case 'hash':
          var r = getKeyDetailsHash(key, redisConnection, res, next);
          return r;
        case 'set':
...
function getKeyDetailsHash (key, redisConnection, res, next) {
  redisConnection.hgetall(new Buffer(key), function (err, fieldsAndValues) {
    if (err) {
    ...
    var details = {
      key: key,
      type: 'hash',
      data: convertBertToString(fieldsAndValues)
    };
    myutil.encodeHTMLEntities(JSON.stringify(details), function (string) {
      res.send(string);
    })
...
// work-capital bert implementation. note that in redis-commander.js, line 131
// {'detect_buffers':true} is on the client start.
function convertBertToString(data) {
  Object.keys(data).forEach(function(place){
        var result = data[place];
        var decoded   = bert_erlastic.decode(result);
        data[place] = bert_erlastic.pp_term(decoded);
      })
      return data;
}
// function convertBertToString(data) {
//   Object.keys(data).forEach(function(place){
//     var result = data[place];
//     var converted = bert.bytes_to_string(result);
//     //var con       = BERT.string_to_bytelist(converted);
//     var decoded   = bert.decode(converted);
//     data[place] = bert.pp_term(decoded);
//   })
//   return data;
// }
